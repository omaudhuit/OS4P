# Visualization of sensitivity analysis
        ...
        **Score = 12 - (12 × cost efficiency ratio / 2000)** when ratio ≤ 2000 EUR/t, **0** otherwise.
        ...
        # LCOE Sensitivity Analysis - this is a completely separate section
        st.subheader("LCOE Sensitivity Analysis")
        
        # Parameter selection for LCOE sensitivity
        st.markdown("Select an energy parameter to analyze its impact on LCOE:")
        ...
        lcoe_param_options = {
            ...
            "energy_degradation_rate": "Energy Degradation Rate"
        }
        
        lcoe_col1, lcoe_col2 = st.columns([2, 3])
        with lcoe_col1:
            selected_lcoe_param = st.selectbox(
                "Energy parameter to analyze:", 
                list(lcoe_param_options.keys()),
                format_func=lambda x: lcoe_param_options[x],
                key="lcoe_param"
            )
            
            # Define range based on selected parameter
            if selected_lcoe_param == "solar_capacity_kw":
                min_val, max_val, step = 5.0, 20.0, 1.0
            elif selected_lcoe_param == "wind_capacity_kw":
                min_val, max_val, step = 1.0, 10.0, 0.5
            elif selected_lcoe_param == "solar_capacity_factor":
                min_val, max_val, step = 0.1, 0.3, 0.01
            elif selected_lcoe_param == "wind_capacity_factor":
                min_val, max_val, step = 0.15, 0.4, 0.02
            elif selected_lcoe_param == "energy_degradation_rate":
                min_val, max_val, step = 0.0, 0.02, 0.002
            
            min_range_lcoe = st.number_input("Minimum LCOE value:", value=min_val, step=step, key="lcoe_min")
            max_range_lcoe = st.number_input("Maximum LCOE value:", value=max_val, step=step, key="lcoe_max")
            num_steps_lcoe = st.number_input("LCOE data points:", value=10, min_value=5, max_value=20, step=1, key="lcoe_steps")
        
        with lcoe_col2:
            if min_range_lcoe >= max_range_lcoe:
                st.error("Minimum value must be less than maximum value!")
            else:
                # Generate range values
                lcoe_range_values = np.linspace(min_range_lcoe, max_range_lcoe, int(num_steps_lcoe))
                
                # Perform sensitivity analysis
                lcoe_sensitivity_results = perform_lcoe_sensitivity_analysis(params, selected_lcoe_param, lcoe_range_values)
                
                # Display data table
                st.markdown("#### LCOE Sensitivity Analysis Results:")
                formatted_df = lcoe_sensitivity_results.copy()
                
                # Format values appropriately
                if selected_lcoe_param in ["solar_capacity_factor", "wind_capacity_factor", "energy_degradation_rate"]:
                    formatted_df["Parameter_Value"] = formatted_df["Parameter_Value"].apply(lambda x: f"{x:.3f}")
                else:
                    formatted_df["Parameter_Value"] = formatted_df["Parameter_Value"].apply(lambda x: f"{x:.1f}")
                    
                formatted_df["LCOE"] = formatted_df["LCOE"].apply(lambda x: f"{x:.3f} €/kWh")
                formatted_df["LCOE_Cents"] = formatted_df["LCOE_Cents"].apply(lambda x: f"{x:.1f} €cents/kWh")
                formatted_df["Annual_Energy"] = formatted_df["Annual_Energy"].apply(lambda x: f"{x:,.0f} kWh")
                
                st.dataframe(formatted_df)
        
                # LCOE sensitivity visualization
                lcoe_chart = create_lcoe_sensitivity_chart(
                    lcoe_sensitivity_results, 
                    lcoe_param_options[selected_lcoe_param]
                )
                st.plotly_chart(lcoe_chart, use_container_width=True)
                
                # Add energy production vs LCOE chart
                st.subheader("Energy Production vs LCOE")
                
                # Create scatter plot of energy production vs LCOE
                fig = px.scatter(
                    lcoe_sensitivity_results,
                    x='Annual_Energy',
                    y='LCOE_Cents',
                    size='Parameter_Value',
                    hover_data=['Parameter_Value'],
                    title=f'Energy Production vs LCOE ({lcoe_param_options[selected_lcoe_param]})',
                    labels={'Annual_Energy': 'Annual Energy Production (kWh)', 
                            'LCOE_Cents': 'LCOE (€cents/kWh)',
                            'Parameter_Value': lcoe_param_options[selected_lcoe_param]}
                )
